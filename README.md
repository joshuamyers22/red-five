...
five notebooks, including [conditional uncertainty](notebooks/uncertainty.ipynb),
[temporal folds and trial history](notebooks/temporal-trials.ipynb)
...
[Uncertainty sections](docs/UNCERTAINTY.md) add explicitly configured block-bootstrap
intervals for each time-series contract or mean per-date cross-sectional IC.
They remain conditional and not selection-adjusted. Real final-assessment dates
and access policy are still [open decisions](docs/FINAL_ASSESSMENT.md), not an
implemented holdout lock.
