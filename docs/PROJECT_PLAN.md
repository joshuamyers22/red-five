...
| 3 | Temporal validation + trial ledger + uncertainty | F02, F03, F15 | Fold-leakage and null/dependence simulations; retained OOS observations | Quant | Phase 1 | Partial: declared fold/local journal development slice below; upstream OOS, nested/final assessment and uncertainty remain unimplemented |
...
The reporting/composition slices add notebook and offline views plus independent section APIs; their 130-test baseline is recorded in [composition evidence](COMPOSITION_VERIFICATION.md). Descriptive fixed-training quantiles and a third notebook are recorded in [quantile verification](QUANTILE_VERIFICATION.md). The latest local extension adds fold audits, a fourth notebook and retained local trial attempts; current executed evidence is in [temporal verification](TEMPORAL_VERIFICATION.md). Statistical inference/selection, breadth, marginal replay, Postgres and scheduling remain subsequent slices. Earlier numerical/production gates remain open; no remote CI or deployment is claimed for the current local edits.
